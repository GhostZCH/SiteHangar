...
const props = defineProps<{ modules: ModuleCard[]; fullscreen?: boolean }>();
...
function buildGraph(R: number, categoryMap: Map<string, CategoryNode[]>) {
  ...
  props.modules.forEach((m, mi) => {
    ...
    const cy = y * cr;
    const cz = Math.sin(theta) * rad * cr;
    ...
    const spread = R * (count > 2 ? 0.28 : 0.36);
    cats.forEach((cat, ci) => {
      ...
      // 分类围绕栏目在三维球内均匀分布
      const ay = cats.length === 1 ? 0 : 1 - (ci / Math.max(cats.length - 1, 1)) * 2;
      const arad = Math.sqrt(Math.max(0, 1 - ay * ay));
      const atheta = golden * ci;
      const ccx = cx + Math.cos(atheta) * arad * spread;
      const ccy = cy + ay * spread;
      const ccz = cz + Math.sin(atheta) * arad * spread;
      ...
      // 文章可能在 cat.links 或 cat.subCategories[].links（兼容两种结构）
      const directLinks = cat.links || [];
      const subCats = cat.subCategories || [];
      const allArticles: { title: string; url: string; subName?: string }[] = [];
      const pickTitle = (l: { title?: string; url: string }) =>
        l.title || decodeURIComponent(l.url.split('/').filter(Boolean).pop() || l.url);
      directLinks.forEach(l => allArticles.push({ title: pickTitle(l), url: l.url }));
      subCats.forEach(sc => (sc.links || []).forEach(l => allArticles.push({ title: pickTitle(l), url: l.url, subName: sc.name })));

      // 分类方向单位向量（从球心指向分类）
      const clen = Math.sqrt(ccx * ccx + ccy * ccy + ccz * ccz) || 1;
      const nx = ccx / clen, ny = ccy / clen, nz = ccz / clen;

      let prevArt = catIdx;
      allArticles.forEach((l, ai) => {
        // 文章节点放置在球面上：沿分类方向 + 大幅随机扰动，让节点更稀疏
        const jx = nx + rand(-1.4, 1.4);
        const jy = ny + rand(-1.4, 1.4);
        const jz = nz + rand(-1.4, 1.4);
        const jlen = Math.sqrt(jx * jx + jy * jy + jz * jz) || 1;
        const radius = R * rand(0.94, 1.0); // 球面附近
        const idx = nodes.length;
        nodes.push({
          x: (jx / jlen) * radius,
          y: (jy / jlen) * radius,
          z: (jz / jlen) * radius,
          size: rand(1.2, 2.0), color: artColor, glow: 4, tw: Math.random() * 6,
          type: 'article',
          label: l.subName ? `${l.subName} · ${l.title}` : l.title,
          link: l.url,
        });
...
function onWheel(e: WheelEvent) {
  e.preventDefault();
  zoom = Math.max(0.4, Math.min(5, zoom * (e.deltaY < 0 ? 1.1 : 0.9)));
}
...
function draw(time: number) {
  ...
  const R = Math.min(W, H) * (props.fullscreen ? 0.46 : 0.36) * zoom;
  ...
  // 节点坐标基于 buildGraph(100) 生成，scale 把单位坐标映射到像素半径
  const scale = R / 100;
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    let y = n.y * cxv - z * sx;
    const depth = FOV / (FOV + z / 100);
    proj[i] = { x: cx + x * depth * scale, y: cy + y * depth * scale, d: depth };
  }
...
<template>
  <div ref="wrapRef" class="column-sphere" :class="{ 'column-sphere-fullscreen': fullscreen }" @wheel.prevent="onWheel">
    <canvas
      ref="canvasRef"
