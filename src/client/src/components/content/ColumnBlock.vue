...
<template>
  <div class="column-layout" :class="getColumnClass(columns.length)">
    <div v-for="(col, idx) in columns" :key="idx" class="column-item">
      ...
      <StatsBlock v-if="col.stats && col.stats.length" :items="col.stats" />
      <ChipsBlock v-if="col.chips && col.chips.length" :items="col.chips" />
      <ChartBlock v-if="col.charts && col.charts.length" :items="col.charts" />
