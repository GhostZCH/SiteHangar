...
function bindImageEvents() {
  if (!descContainer.value) return;
  const images = descContainer.value.querySelectorAll('img.md-image');
  images.forEach((img) => {
...
function onImageClick(ev: Event) {
  ev.preventDefault();
  ev.stopPropagation();
  const src = (ev.currentTarget as HTMLImageElement).getAttribute('src') || '';
  openLightbox(src);
}

function onImageDblClick(ev: Event) {
  ev.preventDefault();
  ev.stopPropagation();
  const src = (ev.currentTarget as HTMLImageElement).getAttribute('src') || '';
  openLightbox(src);
...
onMounted(() => {
  ...
  nextTick(bindImageEvents);
});

watch(() => props.items, () => nextTick(bindImageEvents), { deep: true });
watch(() => katexReady.value, () => nextTick(bindImageEvents));
onUnmounted(() => {
  offKatexReady?.();
});
...
<template>
  <div ref="descContainer" class="section-desc">
    <template v-for="(item, i) in renderedItems" :key="i">
      <h3 v-if="item.headingLevel === 3" class="desc-heading">{{ item.headingText }}</h3>
      <h4 v-else-if="item.headingLevel === 4" class="desc-heading desc-heading-h4">{{ item.headingText }}</h4>
      <template v-else>
        <template v-for="(part, pi) in item.parts" :key="pi">
          <MermaidBlock v-if="part.type === 'mermaid'" :code="part.content" />
          <TreeBlock v-else-if="part.type === 'tree'" :data="part.content" />
