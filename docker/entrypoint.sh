...
# 处理 conf 软链接
if [ -L "$SITE_DIR/conf" ] && [ ! -e "$SITE_DIR/conf" ]; then
    echo "[entrypoint] Conf dir is a symlink, checking target ..."
    if [ -d "/app/my_sites_data/conf" ]; then
        echo "[entrypoint] Re-linking conf to /app/my_sites_data/conf"
        rm -f "$SITE_DIR/conf"
        ln -s /app/my_sites_data/conf "$SITE_DIR/conf"
    else
        echo "[entrypoint] WARNING: /app/my_sites_data/conf not found, conf symlink may be broken"
    fi
fi
