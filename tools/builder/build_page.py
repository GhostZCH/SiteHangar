...
def _strip_ignored_headings(body: str) -> str:
    """
    移除正文中的 ATX 一级标题 (# ) 和二级标题 (## )。
    页面标题和章节标题只来自 frontmatter，Setext 风格标题也不支持。
    """
    lines = body.split('\n')
    result = []
    for line in lines:
        stripped = line.strip()
        # ATX 一级标题 # 标题 和 二级标题 ## 标题
...
def compile_simple(dir_path: str) -> dict:
    """
    编译单文件模式（page.md）。
    解析规则：元信息只来自 frontmatter；正文一级标题、二级标题忽略；
...
def compile_complex(dir_path: str) -> dict:
    ...
    每个章节文件的标题和副标题只来自其 frontmatter 的 title 和 subtitle 字段。
    正文中的一级、二级标题和 Setext 风格标题均被忽略。
    """
