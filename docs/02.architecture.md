...
本文档说明 SiteHangar 的整体架构、技术选型、编译要求、代码模块、核心文件作用、数据流转、API 流程以及多站点映射机制。
...
| 线上部署 | Nginx（托管前端 dist + 反代 API）+ Node.js（systemd 管理） |
...
采用数据和展示分离设计的架构，网页原始数据是在文件系统编译的文本文件（yaml，markdown），通过工具转换成 JSON 文件，网站服务读取这些json进行展示，这个过程叫做编译。功能尽量简化，不支持用户交互，仅展示数据，去掉，注册，登录，点赞，评论等功能。
...
| `src/router/index.ts` | 路由表：home、info、module、detail、graph、404 |
...
| `src/composables/useSiteDetection.ts` | 当前站点检测与模块匹配 |
| `src/composables/useMobileBarAutoHide.ts` | 移动端导航栏自动隐藏 |
...
| `src/views/GraphView.vue` | 图谱页：栏目/分类关系可视化 |
| `src/views/InfoView.vue` | 关于/信息页 |
| `src/views/NotFoundView.vue` | 404 页面 |
| `src/components/content/SectionRenderer.vue` | 章节渲染器，根据内容类型分发到各子组件 |
| `src/components/content/` | 内容块组件：DescriptionBlock、ChartBlock、MermaidBlock、TimelineBlock、TableBlock、StatsBlock、CardListBlock、ChipsBlock、ColumnBlock、ColumnNetworkSphere、ListBlock、TreeBlock、BranchVisualizerBlock、SubSectionBlock、DetailHero、DetailContent、ImageLightbox、PageLoading、PageError |
| `src/components/layout/AppHeader.vue` | 顶部导航栏 |
| `src/components/layout/AppNav.vue` | 左侧抽屉导航 |
| `src/components/layout/HeaderNav.vue` | 顶部导航链接 |
| `src/components/layout/MobileNav.vue` | 移动端导航 |
| `src/components/layout/NavList.vue` | 导航列表渲染 |
| `src/components/layout/ThemeSwitcher.vue` | 主题切换器 |
...
| `src/routes/module.routes.ts` | `/api/sites/:siteSlug/columns`、`:columnSlug/pages`；列出站点栏目和栏目下页面 |
| `src/routes/image.routes.ts` | `/api/image/:site/:filename`（站点图片）、`/api/page-asset/:site/*path`（页面级静态资源） |
| `src/routes/config.routes.ts` | `/api/config`：读取网站公开配置（ICP 等） |
...
| `tools/editor/main.py` | 本地 Markdown 编辑器：独立 Web 服务，支持编辑和实时预览 |
| `tools/editor/static/` | 编辑器前端静态资源（HTML/CSS/JS） |
| `tools/editor/README.md` | 编辑器使用说明 |
