...
## 2026-07-27：首页单栏目布局与标题栏链接动效优化

### 变更内容

- **首页栏目卡片最大宽度限制**：
  - 当栏目数量 ≤ 4 时，网格使用 `is-compact-modules` 类，强制每行最多 4 栏，且单卡片最大宽度限制为 260px，居中排列。
  - 避免单个栏目时卡片被拉伸到全屏的问题，正常打开时呈现接近 4 栏并排的尺寸。
  - 涉及文件：`src/client/src/views/HomeView.vue`、`src/client/src/styles/home-view.css`。

- **标题栏 Powered by 链接呼吸动画**：
  - `appbar.css` 中为 `.appbar-powered a` 添加 `poweredPulse` 呼吸动画，颜色与透明度周期性变化，提示该链接可点击。
  - 鼠标悬停时暂停动画并高亮为主题主色。

### 相关文档

- `src/client/src/views/HomeView.vue`
- `src/client/src/styles/home-view.css`
- `src/client/src/styles/appbar.css`
...
- **废弃 `:::` 围栏块语法**（`::: stats`、`::: cards`、`::: timeline`、`::: branches`、`::: chips`），统一使用 JSON 文件引用 `![type](data/xxx.json)` 或 Markdown Code 块 ```type 提供数据。
- **`tools/builder/build_page_parser.py`**：
  - 移除 `_try_fence_block` 与 `_apply_fence` 方法。
  - 移除 `_parse_stats_block`、`_parse_cards_block`、`_parse_timeline_block`、`_parse_branches_block`、`_parse_chips_block` 五个废弃函数。
- **`tools/editor/static/editor.js`**：移除 `:::` 围栏块预览渲染逻辑。
- **`docs/04 exmd.md`**：更新「扩展内容」表格，删除所有 `:::` 围栏语法说明。

### 技术决策

- `:::` 语法与标准 Markdown 围栏冲突，且不利于 AI 和人类阅读；Code 块形式更通用、工具链支持更好。

### 相关文档

- `docs/04 exmd.md` — EXMD 扩展语法参考

---

## 2026-07-24：修复 TreeBlock 树控件主题适配与文字重叠问题

### 变更内容

- **前端 `src/client/src/components/content/TreeBlock.vue`**：
  - 引入 `useTheme()` 监听当前主题，主题切换时自动重绘树状图
  - 初始化时通过 `getComputedStyle` 读取当前主题下 `--on-surface` 和 `--accent` 的实色值，替换 ECharts 无法解析的 CSS 变量
  - 标题、标签、连线、节点颜色均使用当前主题实际色值，解决深色/浅色主题下文字不可见或颜色错乱的问题
  - 开启 `roam: true` 支持缩放拖拽，并配置 `labelLayout: { hideOverlap: true }` 与标签自动换行，解决节点密集时文字相互叠加的问题
  - 统一字体大小并设置标签最大宽度，提升复杂树状图的可读性

### 技术决策

- ECharts 在 canvas 中绘制时无法解析 CSS 自定义属性，因此需要从计算样式中读取主题色并透传给 ECharts 配置
- 标签重叠问题优先通过 ECharts 的 `hideOverlap` 和 `overflow: 'break'` 解决，同时保留 `roam` 让用户可手动缩放查看完整结构

### 相关文档

- `src/client/src/components/content/TreeBlock.vue`

---

## 2026-07-21：部署文档拆分线上/线下两种方式

### 变更内容

- **重构 `docs/deployment.md`**：
  - 新增"线上部署"章节：安装包制作步骤（数据编译、前端 `vite build`、后端 `tsc`）、服务器安装、Nginx 配置（托管 dist 静态文件 + 反代 `/api/`）、systemd 服务配置与常用命令、线上更新流程
  - 原有 Docker 内容保留为"线下测试"章节，结构编号整理
  - 数据目录约定统一说明：示例数据属项目仓库，线上数据独立存放
  - 新增 FAQ：线上 API 404 时检查 `CONFIG_FILE` 环境变量
- **更新 `docs/architecture.md`**：
  - 技术栈表"部署"拆分为"线上部署"和"线下测试"两行
  - "部署架构"章节拆分为线上（Nginx + Node.js + systemd）与线下（Docker 三进程）两小节
  - 页面渲染流程补充线上路径（dist 静态文件）
- **更新 `README.md`**：定位、技术栈表、架构一句话、快速开始均区分线上/线下两种方式

### 技术决策

- 线上不使用 Docker 和 Vite dev server：前端静态文件由 Nginx 直接托管，后端编译为 JS 由 systemd 管理的 Node 进程运行
- 后端通过 `CONFIG_FILE` 环境变量定位 `config.yaml`，线上数据目录与本地 `my_sites_data` 结构一致
...
## 2026-07-21：精简 project_rules.md

### 变更内容

- 将 `.trae/rules/project_rules.md` 精简为只保留 README 链接和两条核心要求（动手前阅读文档、修改后同步更新文档）
- 删除已过期的文档索引（引用了已被合并删除的 4 份旧文档）
- 分散保留有用内容：
  - "优先使用第三方插件" 移入 `docs/architecture.md` 开发规范
  - 示例数据与线上数据的归属说明移入 `docs/deployment.md` 目录约定
  - 纠正了源数据目录为 `example/`（原文档误写为 `examples/`）
